...
urls_url = 'http://www.image-net.org/api/text/imagenet.synset.geturls?wnid=n02876657'
...
def store_images():
	...
	if not os.path.exists('train'):
		os.makedirs('train')
	if not os.path.exists('train/'+directory_name):
		os.makedirs('train/'+directory_name)

	# Processes the links
	for i in images_urls.split('\n'):
		try:
			print(i)
			urllib.request.urlretrieve(i, os.path.join('train/'+directory_name, "{:04}".format(pic_num)+".jpg"))
			statinfo = os.stat(os.path.join('train/'+directory_name, "{:04}".format(pic_num)+".jpg"))
			if statinfo.st_size > 140:
				img = cv2.imread(os.path.join('train/'+directory_name, "{:04}".format(pic_num)+".jpg"), img_flag)
				img.shape
				resized_image = cv2.resize(img, (img_height, img_width))
				cv2.imwrite(os.path.join('train/'+directory_name, "{:04}".format(pic_num)+".jpg"), resized_image)
				pic_num += 1
		...
		# For the case the image caused an error, it will be removed if it exists as file
			...
			try:
				os.remove(os.path.join('train/'+directory_name, "{:04}".format(pic_num)+".jpg"))
			except:
				pass
...
# Mirrors the images if a larger dataset is wished
def mirror_images():
	global pic_num, directory_name
	pic_num = 1
	for file_type in ['train/'+directory_name]:
		for img in os.listdir(file_type):
			image = cv2.imread(os.path.join('train/'+directory_name, img))
			mirrored = cv2.flip(image, 1)
			cv2.imwrite(os.path.join('train/'+directory_name, 'img'+str(pic_num)+'.jpg'), mirrored)
			pic_num += 1
...
def rename_images():
	...
	# Names the images with one additional digit to avoid overwriting
	for file_type in ['train/'+directory_name]:
		for img in os.listdir(file_type):
			os.rename(str(file_type)+'/'+str(img), str(file_type)+'/'+"{:05}".format(pic_num)+".jpg")
			pic_num += 1

	# Renames the files back to 4 digits
	pic_num = 1
	for file_type in ['train/'+directory_name]:
		for img in os.listdir(file_type):
